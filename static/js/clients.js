...
export async function openClients() {
  try {
    ...
    alert('Не удалось загрузить клиентов: ' + e.message);
  }
}
...
export async function openClientDetail(name) {
  try {
    const data = await api('/api/admin/clients/' + encodeURIComponent(name));
    ...
    alert('Не удалось загрузить клиента: ' + e.message);
  }
}

export async function exportClients() {
  try {
    const data = await api('/api/admin/clients/export');
...
export async function openAudit() {
  ...
  try {
    ...
    alert('Не удалось загрузить аудит: ' + e.message);
  }
}
