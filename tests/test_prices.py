...
def test_list_prices(client, auth_headers, db):
    ...
    db.commit()
    cat_id = db.execute("SELECT id FROM price_categories ORDER BY id DESC LIMIT 1").fetchone()[0]
...
def test_update_category(client, auth_headers, db):
    ...
    row = db.execute("SELECT * FROM price_categories WHERE id = ?", (cat_id,)).fetchone()
...
def test_delete_category(client, auth_headers, db):
    ...
    # Проверяем удаление
    row = db.execute("SELECT id FROM price_categories WHERE id = ?", (cat_id,)).fetchone()
    assert row is None
...
def test_create_item(client, auth_headers, db):
    ...
    # Проверяем создание
    row = db.execute("SELECT * FROM price_items WHERE name = 'Новая Позиция'").fetchone()
    assert row is not None
    assert row["price"] == "2500-3500"
    assert row["unit"] == "м2"


def test_update_item(client, auth_headers, db):
    ...
    db.commit()
    item_id = db.execute("SELECT id FROM price_items ORDER BY id DESC LIMIT 1").fetchone()[0]
    ...
    # Проверяем обновление
    row = db.execute("SELECT * FROM price_items WHERE id = ?", (item_id,)).fetchone()
    assert row["name"] == "Новая Позиция"
    assert row["price"] == "2000-3000"
    assert row["unit"] == "м"


def test_delete_item(client, auth_headers, db):
    ...
    # Проверяем удаление
    row = db.execute("SELECT id FROM price_items WHERE id = ?", (item_id,)).fetchone()
    assert row is None
